        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                try {
                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
                                    m.reply(`!!Error Ocurrido: *Plugin:* ${m.plugin}\n*Sender:* ${m.sender}\n*Chat:* ${m.chat}\n*Command:* ${usedPrefix}${command}`.trim(), data.jid)
                            }
                        m.reply(text)
                    }
                } finally {
                    ...
                    if (m.limit)
                        m.reply(+m.limit + ' 𝘓𝘪𝘮𝘪𝘵𝘦(𝘴) 𝘢𝘱𝘭𝘪𝘤𝘢𝘥𝘰(𝘴) 𝘤𝘰𝘳𝘳𝘦𝘤𝘵𝘢𝘮𝘦𝘯𝘵𝘦 ☑')
                }
                break
            }
        }
